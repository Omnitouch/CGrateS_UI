...
const RatingProfiles = ({ cgratesConfig }) => {
    ...
    const fetchRatingProfileDetails = async (profileString) => {
        ...
        // Split the profile string by colon
    ...
    const handleRowClick = (profileString) => {
        fetchRatingProfileDetails(profileString); // Fetch the details when a profile is clicked
    };
            ...
            <Container>
                ...
                <Modal show={showModal} onHide={handleCloseModal} size="lg">
                    <Modal.Header closeButton>
                        <Modal.Title>Rating Profile Details: {selectedProfile.Id}</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                        {selectedProfile ? (
                            <>
                                ...
                                {selectedProfile.RatingPlanActivations && selectedProfile.RatingPlanActivations.length > 0 ? (
                                    <Accordion defaultActiveKey="0">
                                        {selectedProfile.RatingPlanActivations.map((activation, index) => (
                                            <Accordion.Item eventKey={index.toString()} key={index}>
                                                <Accordion.Header><strong>Rating Plan ID:</strong> {activation.RatingPlanId}</Accordion.Header>
                                                <Accordion.Body>
                                                    <ListGroup variant="flush">
                                                        <ListGroup.Item><strong>Activation Time:</strong> {activation.ActivationTime}</ListGroup.Item>
                                                        {activation.FallbackKeys ? (
                                                            <ListGroup.Item><strong>Fallback Keys:</strong> {activation.FallbackKeys.join(', ')}</ListGroup.Item>
                                                        ) : (
                                                            <ListGroup.Item><strong>Fallback Keys:</strong> None</ListGroup.Item>
                                                        )}
                                                    </ListGroup>
                                                </Accordion.Body>
                                            </Accordion.Item>
                                    ...
                                    <p>No rating plan activations available</p>
                                )}
                            </>
                        ) : (
